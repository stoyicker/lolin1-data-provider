...
public final class NewsService extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        RealmEnum realm;
        if ((realm = convertStringToRealmEnum(parameterRealm)) == null || !localeBelongsTo(parameterLocale, realm)) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        ...
        try {
            resp.getOutputStream().write(DataAccessObject.getJSONNewsAsString(realm, parameterLocale).getBytes
